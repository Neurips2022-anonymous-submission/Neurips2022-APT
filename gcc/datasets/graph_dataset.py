...
def select_graph(dgl_graphs, candidate_indices, model, dgl_file, contrast, epoch, device):
    ...
    for i, graph in enumerate(graphs):
        ...
        # if i == 21:
        #     break
        # dglgraph = dgl.to_bidirected(dgl_graphs[0][graph[0]])
    ...
    # max_idx = candidate_indices[min_idx]
    # print(max_idx)
    # max_idx = -1
    # max_v = -100
    # for i in range(len(final_score)):
    #     if max_v < final_score[i] and i in candidate_indices:
    #         max_v = final_score[i]
    #         max_idx = i
    # max_idx = np.argmax(mean_proerty_init_with_alpha)
    print(proerty_init_)
    return max_idx
...
class LoadBalanceGraphDataset_al_each(torch.utils.data.IterableDataset):
    ...
    def check_uncertainty(self, model, contrast, epoch):
        a = dgl.data.utils.load_graphs(self.dgl_graphs_file)
...
class LoadBalanceGraphDataset_al_each_one(torch.utils.data.IterableDataset):
    def __init__(
            self,
            rw_hops=64,
            restart_prob=0.8,
        ...
        super(LoadBalanceGraphDataset_al_each_one).__init__()
        self.rw_hops = rw_hops
        self.num_neighbors = num_neighbors
        self.restart_prob = restart_prob
        self.positional_embedding_size = positional_embedding_size
